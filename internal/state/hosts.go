...
// automatically.
func (s *State) ForgetHostReport(host string) (bool, error) {
	...
	if len(matches) > 1 {
		...
		return false, fmt.Errorf("host %q matches multiple reports %q; use an exact spelling", host, matches)
	}
	delete(s.HostReports, matches[0])
	return true, nil
}
